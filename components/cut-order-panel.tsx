...
export function CutOrderPanel({ order, onRequestReload }: Props) {
  ...
  const [dialogType, setDialogType] = useState<"location" | "order" | null>(null);
  ...
  >("todos");
  const [bundleLocationFilter, setBundleLocationFilter] = useState<string>("todos");
  ...
  const locationOptions = useMemo(() => {
    const unique = Array.from(
      new Set(bundles.map((bundle) => bundle.currentLocation || "Sin ubicación")),
    );
    return ["todos", ...unique];
  }, [bundles]);
  ...
  const filteredBundles = useMemo(() => {
    return bundles.filter((bundle) => {
      ...
      const matchesStatus =
        bundleStatusFilter === "todos" || normalizedStatus === bundleStatusFilter;
  ...
  const handleMultiConfirm = async (payload: { value: string; bundleIds: string[] }) => {
    ...
    const success = await runBundleAction(
      {
        ...
        destinationCode: normalizedAction === "mover" ? payload.destination : undefined,
        orderNumber: normalizedAction === "asignar" ? payload.orderNumber : undefined,
      },
      successMessage,
    );
  ...
  const handleSplitConfirm = async (amount: number) => {
    if (!activeBundle || !order) return;
    setIsProcessingAction(true);
    try {
      await splitBundle({ bundleId: activeBundle.id, orderId: order.id, sheets: amount });
      setActionFeedback({ type: "success", text: "Bulto dividido correctamente." });
  ...
  if (!order) {
    return (
      <section
        className="relative flex h-full items-center justify-center rounded-md border border-[var(--primary-muted)] bg-white/90 p-6 shadow-sm"
  ...
  return (
    <section className="flex h-full flex-col overflow-hidden bg-white p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-sm text-[var(--primary)]">
            ...
            <div>
              Fecha:{" "}
              <span className="font-medium text-[var(--primary-dark)]">{order.date}</span>
          </div>
        </div>
        </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
              ...
              <select
                ...
                onChange={(event) =>
                  setBundleStatusFilter(
                    event.target.value as "todos" | "disponible" | "asignado" | "utilizado",
                  )
                }
            ...
            <div className="sm:flex-1">
              ...
              <select
                ...
                onChange={(event) => setBundleLocationFilter(event.target.value)}
                className="mt-2 w-full rounded-md border border-[var(--primary-muted)] px-3 py-2 text-sm text-[var(--primary-dark)] focus:border-[var(--primary)] focus:outline-none sm:h-10"
              >
                ...
                filteredBundles.map((bundle) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`relative flex items-center gap-2 text-xs font-medium uppercase ${
      ...
      {bundleDialogConfig && activeBundle ? (
        <UpdateBundleDialog
          key={`${bundleDialogConfig.title}-${bundleDialogConfig.preset ?? "default"}`}
          bundle={activeBundle}
      ...
      {actionFeedback ? (
        <div className="fixed bottom-4 left-4 z-50 max-w-sm rounded-md border px-4 py-3 text-sm shadow-lg"
          data-test-id="feedback-toast"
          style={{ borderColor: actionFeedback.type === "success" ? "#6ee7b7" : "#fecaca", backgroundColor: actionFeedback.type === "success" ? "#ecfdf5" : "#fef2f2" }}
        >
          <div className="flex items-start gap-3">
            <span className={`font-semibold ${actionFeedback.type === "success" ? "text-emerald-700" : "text-rose-700"}`}>
              {actionFeedback.text}
            </span>
            <button
              type="button"
